 ...
 * Requires SANITY_WRITE_TOKEN to patch the document status to 'rejected'.
 */
...
module.exports = async function (req, res) {
  try {
    ...
    try {
      await client.patch(id).set({ status: 'rejected' }).commit({ returnDocuments: false })
    } catch (err) {
      console.error('Sanity patch error (reject):', err)
      res.statusCode = 500
    ...
    console.error('Unexpected error in /api/reject:', err)
    res.statusCode = 500
    res.setHeader('Content-Type', 'application/json')
